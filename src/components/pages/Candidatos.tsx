...
export default function Candidatos({
	...
	const validacionVoto = async () => {
		const response = await fetchFn(
			`/${url_votacion}/estado_voto?email=${
				session && session.user ? session.user.email : ""
			}`
		);

		if (response.error || response.code !== 200) {
			toast.error("Ya votó anteriormente", {
	...
	useEffect(() => {
		if (status === "authenticated") validacionVoto();
	}, [status]);
				...
				<>
					<Modal isOpen={IsOpenModal} setIsOpen={setIsOpenModal}>
						<div className="w-full max-w-md transform overflow-hidden rounded-2xl bg-default-white p-6 text-left align-middle shadow-[4.0px_8.0px_8.0px_rgba(0,0,0,0.38)] transition-all">
							...
							<button
								type="button"
								className="inline-flex mt-4 justify-center rounded-md border bg-primary text-default-white px-4 py-2 text-sm font-medium "
								onClick={() => {
